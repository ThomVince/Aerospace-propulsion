...
while abs((zetas[1]-zetas[0])) > 0.001*abs(zetas[0]):
    ...
    #2
    F,psi = prandtl_mom_loss_factor_flow_angle(xis,psi_t,nb_blades)

        
    #3, 4, 5
    G = circulation_fct(F,psi,x)
    Res, AoAs, Epses = Re_AoA_Eps_Cls_opt(lmbda,G,v_freestream,zeta,R,nb_blades)
    W_times_c = Res * nu
    #6
    a = ax_interf_factor(zeta,psi,Epses)
    a_prime = rot_interf_factor(zeta,psi,Epses,xis/lmbda)
    ...
    I1_dervs, I2_dervs = I_derivatives(xis,G,Epses,psi,lmbda)
    ...
    print("Iteration time : ", time() - start_time,"s")
    
J1_dervs,J2_dervs = J_derivatives(xis,G,Epses,psi,lmbda)
